...
def _load_plugins() -> List[Tuple[str, object]]:
    ...
    if env != "*" or enabled_entry_points:
        ...
        if sys.version_info >= (3, 10):
            # Not using `import importlib.metadata` because that causes an
            # UnboundLocalError on the usage of importlib above.
            from importlib.metadata import entry_points  # novermin

            eps = entry_points(group=group)  # novermin
        elif sys.version_info >= (3, 8):
            from importlib.metadata import entry_points  # novermin

            eps = entry_points().get(group, ())  # novermin
        else:
        ...
        for entry_point in eps:
            ...
            try:
                ...
                logger.debug(
                    "loaded entry point %r", f"{entry_point.name}={entry_point.value}"
                )
