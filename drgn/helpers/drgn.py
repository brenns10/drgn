...
itself. These are mainly useful to drgn's developers. Just like other debug
targets, you must have debuginfo for drgn available to debug it. Just like other
debug targets, you can debug a drgn core dump or a running process. But unlike
other debug targets, you can additionally debug the running drgn process *from
the same process*. When that is happening, we use the following naming
convention:

- ``prog`` - an instance of drgn, which you want to debug
- ``self_prog`` - a :class:`Program` which is debugging the current process
