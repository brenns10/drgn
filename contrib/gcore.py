...
def vma_snapshot(
    page_size: int, task: Object, sps: Sequence[int],
    ...
    def is_stack(start: int, end: int, sps: Sequence[int]) -> Optional[int]:
    ...
    def add_vma(vma: Object) -> None:
        ...
        # Added logic to filter to only include VMAs with stacks.
        segment_start = start
        if always_dump_vma(vma):
            ...
            if sp is not None:
                ...
                # sps is empty - dump all anonymous mappings
                dump_size = end - start
            else:
                dump_size = 0
        elif (
            ...
            and (
                ...
                (vma.vm_pgoff == 0 and vma.vm_flags & VM_READ)
                # Include first page of writable executable mappings to try to
                # capture the dynamic section.
                or (vma.vm_flags & VM_WRITE)
            )
        ):
...
def main(prog: Program, argv: Sequence[str]) -> None:
    ...
    parser.add_argument(
        ...
        dest="stack_only",
        action="store_true",
        help="only include stack segments",
    )
    ...
    sps = all_thread_stacks(task) if args.stack_only else []

    segments, mapped_files = vma_snapshot(page_size, task, sps)
