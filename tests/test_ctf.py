...
def test_file(name):
    return os.path.join(os.path.dirname(__file__), f"ctfdata/{name}")
...
class TestDwarf(TestTypes, TestCase):
    ...
    def setUpClass(cls):
        cls.prog = Program()
        cls.prog.set_core_dump(test_file("core"))
        cls.prog.load_debug_info([test_file("test")])
...
class TestCtf(TestTypes, TestCase):
    ...
    def setUpClass(cls):
        cls.prog = Program()
        cls.prog.set_core_dump(test_file("core"))
        _linux_helper_load_ctf(cls.prog, test_file("test.ctf"))
...
class TestDwarf2Ctf(TestTypes, TestCase):
    ...
    def setUpClass(cls):
        cls.prog = Program()
        cls.prog.set_core_dump(test_file("core"))
        _linux_helper_load_ctf(cls.prog, test_file("test.dwarf2ctf"))
        # fake symbol finder so that we can lookup variable types, despite not
        # having a symbol table
        cls.prog.add_symbol_finder(fake_symbol_finder)
