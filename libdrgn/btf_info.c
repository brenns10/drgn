                     ...
                     size_t btf_data_size, bool main_module_base)
{
	...
	// Load BTF using the main module as base, if requested
	struct btf *btf = NULL;
	if (base)
		btf = btf__new_split(btf_data, (uint32_t) btf_data_size, base);
	else
		btf = btf__new(btf_data, (uint32_t)btf_data_size);
