...
/*
 * Prior to GCC commit 5d24bf3afd1 ("ctf: fix incorrect CTF for
 * multi-dimensional array types") in v14, multidimensional arrays were
 * represented in reverse of their correct order. Since that commit, the
 * ordering is fixed, but there is no way to distinguish whether the given CTF
 * data was generated by a buggy or fixed GCC version. Hopefully a flag will be
 * added to rectify that, but we still need to deal with everything that is
 * generated until that point.
 *
 * CTF does not contain version information about the program which produced it.
 * Most CTF out there was generated by the older GCC version. Newer kernels are
 * generally built with newer GCC versions, so there is a loose correlation
 * between kernel & GCC version. In practice, what we're aiming for here is
 * ensuring that we choose the right behavior for Oracle UEK versions, because
 * it is the only distribution kernel with CTF.
 *
 * GCC 14+ is used for UEK version 8, as well as the upstream tracking
 * "UEK-NEXT" kernels. Assume that any 6.12 or later kernel was built with GCC
 * 14, and thus their multidimensional array orders should not be reversed.
 * Otherwise, assume the bug is present. Allow this to be overridden by an
 * environment variable.
 */
static bool guess_multidimensional_arrays_reversed(struct drgn_program *prog)
{
	const char *env = getenv("DRGN_CTF_MULTIDIMENSIONAL_ARRAYS_REVERSED");
	if (env) {
	...
	return (major == 6 && minor < 12) || major < 6;
}
...
#endif
	info->prog = prog;
	info->bug_reversed_array_indices = guess_multidimensional_arrays_reversed(prog);
