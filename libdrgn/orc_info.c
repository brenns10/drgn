...
static struct drgn_error *drgn_read_builtin_orc(struct drgn_module *module)
{
	...
	// module info.
	DRGN_OBJECT(arch, module->prog);
	DRGN_OBJECT(tmp, module->prog);
	struct drgn_error *err = drgn_object_member(&arch, &module->object, "arch");
	if (err)
		return err;

	err = drgn_object_member(&tmp, &arch, "num_orcs");
	if (err)
		return err;
