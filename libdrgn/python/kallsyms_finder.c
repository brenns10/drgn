...
static PyObject *KallsymsFinder_call(KallsymsFinder *self, PyObject *args, PyObject *kwargs)
{
	...
	flags |= single ? DRGN_FIND_SYM_ONE : 0;
	...
	if (name_obj != Py_None) {
		...
		flags |= DRGN_FIND_SYM_NAME;
		name = PyUnicode_AsUTF8(name_obj);
	}

	drgn_symbol_result_builder_init(&builder, flags);

	err = drgn_kallsyms_symbol_finder(name, address, NULL, flags, self->finder, &builder);
	if (err)
		goto error;
	...
	if (single) {
		...
		PyObject *pysym = Symbol_wrap(symbol,
...
PyObject *KallsymsFinder_new(PyTypeObject *subtype, PyObject *args, PyObject *kwds)
{
