...
static struct drgn_error *
kallsyms_copy_tables(struct drgn_program *prog, struct kallsyms_reader *kr, struct vmcoreinfo *vi)
{
	struct drgn_error *err;
...
static int
kallsyms_lookup(struct kallsyms_registry *kr, const char *name)
{
	for (int i = 0; i < kr->num_syms; i++)
...
static uint64_t
kallsyms_address(struct kallsyms_registry *kr, unsigned int offset)
{
...
struct drgn_error *drgn_kallsyms_load_btf(struct drgn_program *prog)
{
	...
	int index = kallsyms_lookup(kr, "__start_BTF");
	uint64_t start, end;
	if (index < 0) {
		return &drgn_not_found;
	}
	start = kallsyms_address(kr, index);
	index = kallsyms_lookup(kr, "__stop_BTF");
	if (index < 0) {
		return &drgn_not_found;
	}
	end = kallsyms_address(kr, index);
	...
	enum drgn_find_object_flags flags, void *arg, struct drgn_object *ret)
{
	...
	int symbol_idx = kallsyms_lookup(kr, name);
	if (symbol_idx < 0)
		goto check_enum;
	uint64_t symbol_address = kallsyms_address(kr, symbol_idx);
			       ...
			       struct drgn_symbol *ret)
{
	...
	btf_idx = drgn_btf_lookup_def(bf, ret->name, strlen(ret->name));
	if (btf_idx) {
				  ...
				  struct drgn_symbol *ret)
{
	...
	for (int i = 1; i < kr->num_syms; i++) {
		uint64_t sym_addr = kallsyms_address(kr, i);
		if (address < sym_addr) {
			index = i - 1;
			break;
		}
	...
	drgn_symbol_from_kallsyms(kr, index,  ret);
	return true;
}
...
kallsyms_load_addresses(struct drgn_program *prog, struct kallsyms_registry *kr, struct vmcoreinfo *vi)
{
	...
	if (vi->kallsyms_addresses) {
		 ...
		 */
		int stext_idx = kallsyms_lookup(kr,"_stext");
		if (stext_idx < 0) {
			free(addr32);
			return drgn_error_create(
				DRGN_ERROR_OTHER,
...
drgn_program_load_internal_info(struct drgn_program *prog, struct vmcoreinfo *vi)
{
	...
	err = drgn_kallsyms_init(prog, vi);
