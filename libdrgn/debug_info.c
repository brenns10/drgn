						      ...
						      const char *name)
{
	...
	_cleanup_free_ char *key_to_free = it.entry->key;
			       ...
			       bool is_gnu_debugaltlink_file)
{
	...
	if (module->build_id_len == 0) {
		elf_build_id_len = dwelf_elf_gnu_build_id(file->elf,
							  &elf_build_id);
...
#undef FORMAT
	...
	if (module->debug_file_status == DRGN_MODULE_FILE_WANT_SUPPLEMENTARY
		...
		elf_build_id_len = dwelf_elf_gnu_build_id(elf, &elf_build_id);
		if (elf_build_id_len < 0) {
			     ...
			     size_t n, bool load_default, bool load_main)
{
	...
	for (size_t i = 0; i < num_wanted_modules; i++)
		load_debug_info_log_missing(wanted_modules[i]);
